...
Core syntax and common patterns for Marp slide creation. Generic reference — works across all repos.
...
**Dark theme tip:** Ensure table styling includes background colors for `th` and `td` — transparent backgrounds make text invisible on dark slides.
...
Add explicit table styling in front-matter:
```markdown
style: |
...
Split into multiple slides. Aim for 5–7 bullet points or 6–7 table rows max per slide.

### Images Not Loading in HTML
Use paths relative to the HTML output file location, or absolute paths. Keep images in the same directory as the HTML when possible.

### GitHub Not Rendering .marp.md
GitHub doesn't recognize `.marp.md`. Always generate HTML and share the GitHub Pages URL or raw HTML file.
